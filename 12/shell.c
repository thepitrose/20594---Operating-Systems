...
int runCommand(struct job newJob, struct jobSet * jobList, 
	...
	for (job=jobList->head; job ; job = job->next)     // run for all the jobs
		{
		 if (job->numProgs == job->stoppedProgs)
			{
			  statusString = "Stopped"; // check if stop - print Stopped
			}
		else
			{
			   statusString = "Running"; // else - print Running
			}
	...
	int found=1;            //variable for - if the job not find return 1
	char *commandvar;       //variable for - if the variables are proper
	char *spceinline = "";  //variable for - if we have a space after the command

	if (newJob.progs[0].argv[1]==NULL) // check if the input command is not null
	{
	         fprintf(stderr,"the command is null\n"); // display error  
		   return 1;
	}
       
             commandvar=newJob.progs[0].argv[1]+1; //cast to - check if the variables are proper  
             spceinline=newJob.progs[0].argv[2];  //cast to - if we have a space after the command
     

      for(i=0;commandvar[i]!='\0';i++)  // check if the variables are proper one by one  
      {
      
        if (commandvar[i]<49 || commandvar[i]>57) // check if the are number by the ascii code 1=49(ascii) and 1=57(ascii)
        {
               fprintf(stderr,"the input  - %c  - is incoreect \n",commandvar[i]); // display error
              return 1;   
        }
      }
       
      if (spceinline!=NULL) //check if we have a space after the command
      {
        fprintf (stderr,"the command is not ''clean'' after the variables, you get -  %s  in our line\n",spceinline);  // display error
        return 1;        
      }

	if ((newJob.progs[0].argv[1][0]!='%') || (newJob.progs[0].argv[1][1]=='\0') ) // check if % if the first char or if only  % 
		{
		 perror("no argument or only just a % "); // display error
		   return 1;
		}
         
	jobNum=atoi(newJob.progs[0].argv[1]+1); // get the job id in to temp variable
	
	for (job=jobList->head; job ; job = job->next) // check all the jobs in the list
	{
		if (job->jobId == jobNum) // if find the job
			{
			found=0; //if the job find , the found variable will change to - 0 - for the end return

			
			if (newJob.progs[0].argv[0][0]=='f')  // check if the job is fg
			  { 
			  jobList->fg=job;

				if (tcsetpgrp(0,job->pgrp))
					{
					  perror("tcsetpgrp"); // display error
					}
			  }

			if(kill(-job->pgrp, SIGCONT))
				{
				 perror("cannot continue job, we get kill"); // display error
				}
			job->stoppedProgs=0;
	...
	if (found==1) // if the job not find return 1
	{
	    fprintf(stderr,"%s : is unknown job %d \n",newJob.progs[0].argv[0],jobNum);  // display error
	   return 1;
	}
